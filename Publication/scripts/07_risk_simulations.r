#* 7: Risk Simulations
  #+ 7.1: Setup and preparation
    #- 7.1.1: Set model shortcut
      model_obj <- lasso_all_variants$weighted
    #- 7.1.2: Fit Platt scaling model from CV predictions
  ...
  #+ 7.2: Generate prediction grid and model coefficients
    #- 7.2.1: Generate prediction grid (ASA = 0 and 1)
    ...
    #- 7.2.2: Extract coefficients from weighted model
  ...
  #+ 7.3: Compute predictions and prepare plot data
    #- 7.3.1: Compute predicted probabilities
    ...
    #- 7.3.2: Filter and prepare data for plotting
  ...
  #+ 7.4: Generate visualization plots to compare Platt scaled versus uncalibrated probabilities
    #- 7.4.1: Plot uncalibrated probabilities
    ...
    #- 7.4.2: Plot Platt-scaled probabilities
      ggplot(plot_data, aes(x = injury_grade, y = platt_scaled_prob, color = scenario, linetype = line_type)) +
        ...
        scale_x_continuous(breaks = 0:5) +
        theme_minimal(base_size = 14)
  #+ 7.5: Export data, plot in Prism
    write.csv(plot_data, "stroke_risk_weighted_predictions.csv", row.names = FALSE)
  #+ 7.6: Prepare models to be exported for risk calculator
