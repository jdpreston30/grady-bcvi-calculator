#* 2: Descriptive Statistics
  ...
  #+ 2.2: Compute cohort descriptive statistics
      ...
      #_Now make a summary to join with main later
        injury_summary <- injury_counts %>%
          ...
          mutate(
            ...
            Value = paste0(n, " (", round(pct, 0), "%)")
          ) %>%
          ungroup() %>%
          mutate(
            Grade = stringr::str_remove(Grade, "^[CV]"),
            Variable = paste0(Vessel, " Grade ", Grade)
          ) %>%
          ...
          bind_rows(
            injury_counts %>%
              summarise(
                Carotid = sum(c_across(Cg1:Cg5), na.rm = TRUE),
                Vertebral = sum(c_across(Vg1:Vg5), na.rm = TRUE)
              ) %>%
              pivot_longer(everything(), names_to = "Vessel", values_to = "n") %>%
              mutate(
                total = sum(n),
                pct = round(n / total * 100, 1),
                Variable = paste0(Vessel, " Total"),
                Value = paste0(n, " (", pct, "%)")
              ) %>%
              select(Variable, Value)
          )
    ...
    #- 2.2.5: Define desired order of variables
      desired_order <- c(
        "age", "BMI", "sexM: Y", "race: Black", "race: White", "race: Other or Unknown",
        "SBP", "DBP", "HR",
        "CC_simplified: MVA", "CC_simplified: GLF", "CC_simplified: PvA",
        "CC_simplified: FFH", "CC_simplified: MCC", "CC_simplified: Bicycle",
        "CC_simplified: Assault", "CC_simplified: Other or NS",
        "GCS", "ISS", "AIS_head", "AIS_neck", "AIS_spine", "AIS_face",
        "max_carotid", "max_vert",
        "BLC: Y", "BLV: Y",
        "isolated_C: Y", "isolated_V: Y", "concom_CV: Y",
        "stroke: Y", "ASA: Y", "DC_status: Alive",
        "ICU_days", "vent_days", "THD"
      )
    #- 2.2.6: Run ternD to get descriptive statistics, format custom
      T1 <- ternD(
        data = full_descriptive,
        exclude_vars = "ID"
      ) %>%
        filter(!grepl(": N$", Variable)) %>%
        mutate(
          # Apply rounding to selected Mean_SD values
          Mean_SD = case_when(
            Variable %in% c("age", "BMI", "SBP", "DBP", "HR") & !is.na(Mean_SD) ~ {
              # Extract and round mean ± sd
              matches <- stringr::str_match(Mean_SD, "([0-9.]+) ± ([0-9.]+)")
              paste0(
                round(as.numeric(matches[, 2])), " ± ", round(as.numeric(matches[, 3]))
              )
            },
            TRUE ~ Mean_SD
          ),
          Value = case_when(
            !is.na(n_pct) ~ n_pct,
            Variable %in% c("GCS", "ISS", "AIS_head", "AIS_neck", "AIS_spine", "AIS_face", "max_carotid", "max_vert", "ICU_days", "vent_days", "THD") ~ Median_IQR,
            TRUE ~ Mean_SD
          )
        ) %>%
        filter(Variable %in% desired_order) %>%  # drop any extras if needed
        mutate(Variable = factor(Variable, levels = desired_order)) %>%
        arrange(Variable) %>%
        select(-c(n_pct, Mean_SD, Median_IQR)) %>%
        mutate(rownum = row_number()) %>%
          {
            max_carotid_idx <- which(.$Variable == "max_carotid")
            max_vert_idx <- which(.$Variable == "max_vert")
            carotid_injuries <- injury_summary %>%
              filter(str_starts(Variable, "Carotid"))
            vertebral_injuries <- injury_summary %>%
              filter(str_starts(Variable, "Vertebral"))
            bind_rows(
              dplyr::slice(., 1:max_carotid_idx),
              carotid_injuries,
              dplyr::slice(., (max_carotid_idx + 1):max_vert_idx),
              vertebral_injuries,
              dplyr::slice(., (max_vert_idx + 1):n())
            )
          } %>%
          select(-rownum)
    #- 2.2.7: Export the descriptive statistics
      write.xlsx(T1, "T1.xlsx")
  #+ 2.3: Run ternG on only the carotid injuries
    #- 2.3.1: Filter to carotid and vertebral
    ...
    #- 2.3.2: Compute the descriptive n's separately
    ...
    #- 2.3.3: Run ternG for carotid results
    ...
    #- 2.3.4: Run ternG for vertebral results
    ...
    #- 2.3.5: Combine the results
      combined_results <- bind_rows(
        ...
        select(-p,-OR) %>%
        arrange(desc(Vessel))      
    #- 2.3.6: Export as excel sheet
      write.xlsx(combined_results, "ST1.xlsx")
  #+ 2.4: Run a GLM on the full dataset
    #- 2.4.1: Create imputed version of descriptive_data
      descriptive_data_imputed <- raw_modeling %>%
        ...
        arrange(desc(stroke))
    #- 2.4.1: Create the model formula
      full_model <- glm(
    ...
    #- 2.4.2: Run stepAIC to pare down to simplified model
      adjust_model <- stepAIC(full_model, direction = "both")
    #- 2.4.3: Compute ORs and 95% CIs
