plot_asa_benefit_mobile <- function(coefs_tbl, platt_model, patient_data) {
#* Setup
...
#* Plot
  ...
  #+ Background grid lines
    ggplot2::geom_segment(data = data.frame(y = seq(0, 75, 25)), aes(x = 1, xend = 5, y = y, yend = y), inherit.aes = FALSE, color = "#D6D6D6", linewidth = 0.25) +
    ggplot2::geom_segment(data = data.frame(x = 1:5), aes(x = x, xend = x, y = 0, yend = 75), inherit.aes = FALSE, color = "#D6D6D6", linewidth = 0.25) +
    ggplot2::geom_segment(aes(x = 0.8, xend = 1, y = 0, yend = 0), color = "#D6D6D6", linewidth = 0.25, inherit.aes = FALSE) +
    geom_segment(
      aes(x = 0.805, xend = 0.995, y = 0, yend = 0),
      color = "white",
      linewidth = 2,
      inherit.aes = FALSE
    ) +
    # Prediction curves
    ggplot2::geom_smooth(method = "loess", formula = y ~ x, span = 1.5, se = FALSE, color = "black", linewidth = 1) +
    # Dots for patient
    ggplot2::geom_point(data = asa_risks %>% dplyr::filter(ASA == 0), aes(x = injury_grade, y = scaled_prob), inherit.aes = FALSE, linewidth = 3, shape = 21, fill = "black", color = "black", stroke = 1) +
    ggplot2::geom_point(data = asa_risks %>% dplyr::filter(ASA == 1), aes(x = injury_grade, y = scaled_prob), inherit.aes = FALSE, linewidth = 3, shape = 21, fill = "white", color = "black", stroke = 1) +
  #+ Baseline text and connector
    ...
    ), size = 3.0, fontface = "bold", vjust = 0, lineheight = 0.9) +
  ...
  #+ ASA 1 text and connector
    ...
    ), size = 3.0, fontface = "bold", vjust = 1, lineheight = 0.9) +
  ...
  #+ Axes & Theme
    ggplot2::scale_x_continuous(name = "Max Biffl Grade", breaks = 1:5, limits = c(0.8, 5.8), expand = c(0, 0), guide = "prism_offset")+
    ggplot2::scale_y_continuous(name = "              Predicted Stroke Risk (%)", breaks = seq(0, 75, 25), limits = c(-20, 75), expand = c(0, 0), guide = "prism_offset") +
